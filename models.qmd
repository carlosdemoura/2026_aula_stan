...
$$\log(\mu_i) = \log(n_i) + \alpha + \beta_{\text{idade}[i]} + \gamma_{\text{cidade}[i]}$$
...
O modelo ajustado assume que os tempos de falha dos isoladores seguem uma distribuição de Weibull com parâmetro de forma $\alpha$ e parâmetro de escala $\sigma$, de modo que a função de sobrevivência é dada por $S(t) = \exp\left\{-\left(t/\sigma\right)^{\alpha}\right\}$. As distribuições a priori adotadas foram $\alpha \sim \text{Gamma}(0.1, 0.1)$ e $\sigma \sim \text{Gamma}(0.1, 0.1)$, ambas pouco informativas. A verossimilhança é construída observação a observação: para os isoladores que falharam, a contribuição é a densidade de Weibull avaliada no tempo de falha; para os isoladores censurados às 2729 horas, a contribuição é $S(t) = P(T > t)$, isto é, a probabilidade de o isolador ter sobrevivido além do tempo de censura. Ao combinar essas contribuições com as distribuições a priori, o algoritmo de amostragem obtém a distribuição posterior de $\alpha$ e $\sigma$, a partir da qual é possível derivar a distribuição posterior da própria curva de sobrevivência em qualquer ponto do tempo.
